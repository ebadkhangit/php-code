...
// sub category id set in session for filtration product
 function getSubCat(prCatid,catValue,typecat){
//alert(typecat);	
 //alert(prCatid);
//alert(catValue);
if(typecat=="subCatName"){ prSubCatid= catValue;}else{prSubCatid= "";}
...
if(typecat=="size"){ prSize= catValue;}else{prSize= "";}
			...
			if(xobj)
			{	              
			  xobj.open("GET","store-session-value-filterdata.php?catid="+prCatid+"&subcatid="+prSubCatid+"&subcat_color="+prColor+"&subcat_price="+prPrice+"&subcat_size="+prSize+"&cat_type="+typecat);			
			  xobj.onreadystatechange=function() {
				 if(xobj.readyState==4 && xobj.status==200) {
                               //alert(xobj.responseText);
...
function showSelectPro(){
 ...
 // search products by users home page
	    ...
	    document.getElementById('viewSearchData').innerHTML=xobj.responseText;     	  
		   ...
		   // search products by users home page
		       function fetch_clik_link_products(prdid,type){
		     //  alert(type);
		     // alert(prdid);
		       if(type=="cat")	{		
				...
				window.location.href=prdid;		  
			 	...
			 	window.location.href="product-description.php?prdid="+prdid; 
			 		 }
		 		 }
